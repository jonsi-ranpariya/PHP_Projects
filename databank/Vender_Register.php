		...
		<div class="container-fluid">
				...
				<div class="card-body" id="getdata">	
					...
					<div id="addTable">
						<table id="tabledata" class="table">
							...
							<tbody>
								<?php
									$sql = "SELECT Sr_no,Status,Follow_Up_By,format(Entry_Date, 'dd-MMM-y') as Entry_Date,format(Date_Applied, 'dd-MMM-y') as Date_Applied,Customer,Place,Website_Registration,Certificate_No,Vendor_Code,format(Validity_Date, 'dd-MMM-y') as Validity_Date1,Remarks FROM Data order by Sr_no desc";
			...
			/*validity expire hightligt row*/
								...
								<tr class="<?php echo $rowHighlight; ?>">
									...
									<td data-bs-toggle="tooltip" data-bs-placement="right"  title="<?php echo $row['Entry_Date'];?>"><?php echo substr($row['Entry_Date'], 0, 7);?></td>
									...
									<td><input type="date" name="date[]" class="form-control" required>
										...
										<td>
											<select name="Follow_By[]" class="form-control Follow" id="1Follow" required>
												<option selected="true" disabled="true" value="">SELECT</option>
													<?php
													$sql4="SELECT Distinct sortname FROM [user] where sortname is not NULL order by sortname asc";
													$run4=sqlsrv_query($connt,$sql4);
													while ($row4 = sqlsrv_fetch_array($run4, SQLSRV_FETCH_ASSOC)) { ?>
													<option value="<?php echo $row4['sortname']; ?>"><?php echo $row4['sortname']; ?></option>
													<?php } ?>
											</select>
										</td>
