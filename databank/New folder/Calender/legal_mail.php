...
require 'D:\xampp\htdocs\databank\package\phpmailer\class.phpmailer.php';
    ...
    $sql1 = "SELECT DISTINCT a.Department,b.email FROM legal a left outer join l_Department b on a.Department =      b.department where Status <> 'Complete' and Company_Due_Date < GETDATE()";
		$run1 = sqlsrv_query($con,$sql1);
		while($row1 = sqlsrv_fetch_array($run1, SQLSRV_FETCH_ASSOC))
		{	
		$sql = "SELECT Compliance,Category,Department,FORMAT(Company_Due_Date,'dd-MMM-yy') as DueDate,Status FROM legal 
...
<tr>
<th style="text-align:left;font-size:18px;">Legal (Date Expired) </th>
<th style="text-align:right;font-size:18px;">'. date("d-M-y") .'</th>
	...
	$file_name = 'LegalPendingList.pdf';
	$html_code = $output;
	$pdf = new Pdf();
	...
	$mail->Subject = 'legal Pending Mail';			  //Sets the Subject of the message
	$mail->Body = 'Note : Legal validity expired.';	//An HTML or plain text message body
	if($mail->Send())								 //Send an Email. Return true on success or false on error
	{
		echo '<label class="text-success">legal Details has been send successfully...</label>';
	}
