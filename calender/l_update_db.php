...
// $rowx = sqlsrv_fetch_array($runx,SQLSRV_FETCH_ASSOC);
  ...
  $reciveby = $_POST['reciveby'];
    ...
    $imgname = time().$val;
      if ($imgExt != '') {
      	move_uploaded_file($_FILES['doc']['tmp_name'][$key],'legal_Doc/'.$imgname);
				$sql1 = "INSERT INTO L_upload(file_source,iid,Due_Date) VALUES ('".$imgname."','$sr_no',' $Due_Date')";
				$run1 = sqlsrv_query($con,$sql1);
      }  

	}
  	 	$sql = "UPDATE legal SET Compliance = '$Compliance',Category ='$Category',Department ='$depnt',Due_Date ='$Due_Date',Company_Due_Date ='$c_date',Status ='$Status',Filed_Date ='$f_date',Prepared_By ='$prepardby',Reviewed_By ='$reciveby' WHERE Sr_no = '$sr_no'";	
